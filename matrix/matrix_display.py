...
class MatrixDisplay:
    ...
    def _draw_bounding_box(self, bounding_box: BoundingBox, **kwargs) -> None:
        ...
        for x in range(bounding_box.x1, bounding_box.x2 + 1):
            self.offscreen_canvas.SetPixel(x, bounding_box.y1, colour.red, colour.green, colour.blue)
            self.offscreen_canvas.SetPixel(x, bounding_box.y2, colour.red, colour.green, colour.blue)
        
        # Draw the left and right vertical lines
        for y in range(bounding_box.y1, bounding_box.y2 + 1):
            self.offscreen_canvas.SetPixel(bounding_box.x1, y, colour.red, colour.green, colour.blue)
            self.offscreen_canvas.SetPixel(bounding_box.x2, y, colour.red, colour.green, colour.blue)        

    def calculate_bounding_box(self, text: str, x: int, y: int, centered: bool = False, line_spacing: int = 2) -> Tuple[BoundingBox, List[str], int]:
        ...
        total_height = len(wrapped_text) * self.font.height + (len(wrapped_text) - 1) * line_spacing
    ...
    def store_bounding_box(self, bounding_box: BoundingBox, box_key: Tuple[int, int, str]) -> bool:
        """Check for overlaps and store the bounding box if valid."""
        if any(bounding_box.overlaps(existing_box) for existing_box in self.bounding_boxes.values()) or not bounding_box.is_within_screen_bounds(self.matrix.width, self.matrix.height):
            return False
        else:
    ...
    def draw_text(self, x: int, y: int, text: str, colour: graphics.Color):
        ...
        if box_key in self.bounding_boxes:
            ...
            bounding_box, wrapped_text, total_height = self.calculate_bounding_box(text, x, y)
    ...
    def draw_centered_text(self, text: str, color: graphics.Color, **kwargs) -> None:
        ...
        start_y = kwargs.get("start_y", (self.matrix.height - len(wrapped_text) * (self.font.height + self.LINE_SPACING)) // 2 + self.font.height)
        box_key = ('c', 'c', text)

        if box_key in self.bounding_boxes:
            bounding_box = self.bounding_boxes[box_key]
        else:
            bounding_box, wrapped_text, total_height = self.calculate_bounding_box(text, 0, start_y, centered=True)
            if not self.store_bounding_box(bounding_box, box_key):
                self._draw_bounding_box(bounding_box, colour=Colours.RED)
                return
