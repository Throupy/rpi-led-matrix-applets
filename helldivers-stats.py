...
def load_and_convert_image(image_path):
    """Load the image and apply coloring"""
...
def fetch_data():
    """Fetch data from helldivers API"""
    ...
    try:
        ...
        print("Looks like we hit the rate limit")
...
def update_display(image, text):
    ...
    text_width = graphics.DrawText(
        matrix, font, 0, 0, color, text)  # Measure text width
...
try:
    while True:
        ...
        if current_time - last_switch_time >= 5:
            current_image = image_bots if current_image == image_bugs\
                else image_bugs
